...
\begin{align}
  & \phantom{=}\, \frac{\mathrm{PSD}_{x} + \mathrm{PSD}_{y} - \mathrm{PSD}_{\mathrm{diff}}}{\mathrm{PSD}_{x} + \mathrm{PSD}_{y}}& \\ \label{eq:diff_index}
...
\end{align}
ここで、3行目で最も寄与の大きい$\tau=0$の値をとって近似した。この時、分子に$x(t)$と$y(t+\delta t)$の相関に当たる項が残り、2つのTODの相互相関関数に対応する。つまり、2つのTODが時間差$\delta t$で相関が大きい時に、この値は大きくなる。逆に相関が小さい時は0に近づく。また、分母で規格化されているため、相関が最大の時に値は1をとり、完全に相関がなければ0になる。この性質から、式\eqref{eq:diff_index}を指標としてTODの相関を測ることとする。

\subsection{timing offsetの算出}
次に、各検出器間で異なるtiming offset($\delta t$)を計算する。GroundBIRDは仰角を固定して方位角方向にスキャンを行うため、検出器間での方位角のずれ(オフセット)をスキャン角速度で割った時間がスキャンにかかるtiming offsetになる。
