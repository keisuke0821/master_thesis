...
\begin{figure}[htbp]
  ...
  \label{timing_offset}
\end{figure}
オフセットは仰角に由来する球面での縮尺(1/$\cos(\mathrm{El})$で拡大される)の効果を受ける。そのため、仰角$\SI{70}{^\circ}$を中心で観測すると隣接する検出器間での方位角オフセットは$\sim\SI{2.5}{^\circ}$になり、$\delta t\sim\SI{45}{\mathrm{ms}}$になる。
