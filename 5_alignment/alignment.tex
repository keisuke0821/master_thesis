...
\begin{equation}
  F(f) = \displaystyle\sum_{n=-\infty}^{\infty}x(n)e^{-2\pi ifn}
\end{equation}
とし、PSD$(f)$は$|F(f)|^{2}$で与えられる。ここで、自己相関関数のフーリエ変換を考えると
\begin{align}
  F(R_{xx}(k)) &= \displaystyle\sum_{k=-\infty}^{\infty}\biggl\{\displaystyle\sum_{n=-\infty}^{\infty}x(n)x(n+k)\biggr\}e^{-2\pi ifk} \\
