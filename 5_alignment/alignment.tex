...
仰角のデータ取得が安定になったことで次に取り組むべき課題が明らかになった。GroundBIRD実験での偏光測定のためには、検出器間での信号の差分を取ることが重要であり、それに伴って望遠鏡のスキャンに対して最適な検出器のアライメントが求められる。この章では焦点面検出器アライメントが抱える課題の定量的な評価を行い、較正に向けた方針を提示する。
