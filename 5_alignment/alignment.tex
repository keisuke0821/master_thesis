...
\begin{equation}
  F(f) = \mathcal{F}[x(t)] = \int_{-\infty}^{\infty}x(t)e^{-2\pi ift}
\end{equation}
とし、PSD$(f)$は
\begin{equation}
...
\begin{align}
  \mathcal{F}[R_{xx}(\tau)] &= \int_{-\infty}^{\infty}\biggl\{\lim_{T\to \infty}\frac{1}{T}\int_{-T/2}^{T/2}x(t)x(t+\tau)dt\biggr\}e^{-2\pi if\tau}d\tau \\
               &= \lim_{T\to \infty}\frac{1}{T}\biggl\{\int_{-T/2}^{T/2}x(t)dt\biggr\}\biggl\{\int_{-\infty}^{\infty}x(t+\tau)e^{-2\pi ift}d\tau\biggr\} \\
