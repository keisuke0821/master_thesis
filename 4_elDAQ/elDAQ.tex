...
テスト3と4では直流電源(松定プレシジョン製)を使用し、与える電圧値を変えて稼働の安定性を確認した。USBの定格電圧が5Vであることと、Zyboで推奨されている供給電圧が4.5V$\sim$5.5Vであることから5.0Vと4.5Vを設定した。電源を定電圧モードにすることで電流値はZyboでデータ取得システムを動かすために必要な電流量になる。テスト3では5.0Vで行い、安定した稼働結果と電流値$\sim$0.36Aを得た。
