...
GroundBIRDでは高速スキャンのもとで角度分解能を失わないようにサンプリングレートを1kHzにしている。そのため、検出器の応答時間が$\mathcal{O}$(ms)であることが要求される。MKIDの典型的な応答時間は1m秒以下であり、この要求を満たしている。CMB観測実験では検出器として``TES(Transition Edge Sensor)''ボロメータを使うこともあるが、GroundBIRDでは時間応答性の面からMKIDを採用している。
\subsection{物理ターゲット}
